...
> **Note:** `data/` CSV files are not committed (too large — 133 MB). They are downloaded automatically when you run notebook cell 1. `models/fashion_model.h5` is generated by the notebook — download it from Colab and place it in `models/` before starting the API.
